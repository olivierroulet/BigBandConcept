...
CREATE VIEW `v_activeuserroles`  AS  select `userroles`.`UR_ID` AS `UR_ID`,`userroles`.`UR_RoleDescr` AS `UR_RoleDescr`,`userroles`.`UR_ActiveYN` AS `UR_ActiveYN` from `userroles` where (`userroles`.`UR_ActiveYN` = 1) ;
